...
// combined cloud cost view
CREATE OR REPLACE VIEW
  SNOWALERT.BI.HASHMAP_COMBINED_CLOUD_COSTS
AS (
  SELECT 
    'AWS' AS SERVICE, 
    LINE_ITEM_UNBLENDED_COST AS COST, 
    LINE_ITEM_USAGE_START_DATE::TIMESTAMP_TZ AS DATE 
  FROM SNOWALERT.BI.HASHMAP_AWS_COSTS
  WHERE LINE_ITEM_LINE_ITEM_TYPE = 'Usage'
) UNION ALL (
  SELECT 
    'Azure' AS SERVICE, 
    PRE_TAX_COST AS COST, 
    USAGE_DATE_TIME::TIMESTAMP_TZ AS DATE 
  FROM SNOWALERT.BI.HASHMAP_AZURE_COSTS
) UNION ALL (
  SELECT 
    'Snowflake Compute' AS SERVICE, 
    CREDITS_USED * 3.00 AS COST, // convert credits to dollars here using the $3/credit enterprise rate
    START_TIME::TIMESTAMP_TZ AS DATE 
  FROM SNOWALERT.BI.WAREHOUSE_METERING_HISTORY
)
//===========================================================